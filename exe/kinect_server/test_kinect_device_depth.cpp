...
typedef KinectDepthImageMessage<PNGImageMessage<> > _ImageMsg;

int main()
{
    std::ofstream output_stream1( "test_kinect_depth.bin", std::ios::binary );
    std::ofstream output_stream2( "test_kinect_depth.png", std::ios::binary );
    std::ofstream output_stream3( "test_kinect_depth.pak", std::ios::binary );
    ...
    while( true )
    {
        try
        {
            kinect_device.pullDepthImage( output_message_ptr );
            break;
        }
        catch( KinectException & e )
        {
